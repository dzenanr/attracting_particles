...
$$.Particle = {"":
 ["radius?", "vy=", "vx=", "y=", "x=", "context", "canvas"],
 super: "Object",
 draw$0: function() {
  var t1 = this.context;
  t1.set$fillStyle('white');
  t1.beginPath$0();
  t1.arc$6(this.x, this.y, this.radius, 0, 6.283185307179586, false);
  t1.fill$0();
 },
 Particle$2: function(canvas, context) {
  var t1 = $.Math_random();
  var t2 = this.canvas;
  this.x = $.mul(t1, t2.get$width());
  this.y = $.mul($.Math_random(), t2.get$height());
  var t3 = $.mul($.Math_random(), 2);
  if (typeof t3 !== 'number') throw $.iae(t3);
  this.vx = -1 + t3;
  var t4 = $.mul($.Math_random(), 2);
...
$.update = function() {
  for (var i = 0; $.ltB(i, $.get$length($.particles)); ++i) {
    var p = $.index($.particles, i);
    p.set$x($.add(p.get$x(), p.get$vx()));
    p.set$y($.add(p.get$y(), p.get$vy()));
    if ($.gtB($.add(p.get$x(), p.get$radius()), $.canvas.get$width())) p.set$x(p.get$radius());
    else {
      $.ltB($.sub(p.get$x(), p.get$radius()), 0) && p.set$x($.sub($.canvas.get$width(), p.get$radius()));
    }
    if ($.gtB($.add(p.get$y(), p.get$radius()), $.canvas.get$height())) p.set$y(p.get$radius());
    else {
      $.ltB($.sub(p.get$y(), p.get$radius()), 0) && p.set$y($.sub($.canvas.get$height(), p.get$radius()));
    }
    for (var j = i + 1; $.ltB(j, $.get$length($.particles)); ++j) {
      $.distance(p, $.index($.particles, j));
    }
  }
};
...
$.paintBlack = function() {
  $.context.set$fillStyle('rgba(0, 0, 0, 1)');
...
$.Particle$ = function(canvas, context) {
  var t1 = new $.Particle(null, null, null, null, null, context, canvas);
  t1.Particle$2(canvas, context);
  return t1;
};
...
$.distance = function(p1, p2) {
  var dx = $.sub(p1.get$x(), p2.get$x());
  var dy = $.sub(p1.get$y(), p2.get$y());
  var dist = $.Math_sqrt($.add($.mul(dx, dx), $.mul(dy, dy)));
  if ($.leB(dist, $.minDist)) {
    $.context.beginPath$0();
    var t1 = $.div(dist, $.minDist);
    if (typeof t1 !== 'number') throw $.iae(t1);
    var t2 = 'rgba(255, 255, 255, ' + $.S(1.2 - t1) + ')';
    $.context.set$strokeStyle(t2);
    $.context.moveTo$2(p1.get$x(), p1.get$y());
    $.context.lineTo$2(p2.get$x(), p2.get$y());
    $.context.stroke$0();
    var ax = $.div(dx, 2000);
    var ay = $.div(dy, 2000);
    p1.set$vx($.sub(p1.get$vx(), ax));
    p1.set$vy($.sub(p1.get$vy(), ay));
    p2.set$vx($.add(p2.get$vx(), ax));
    p2.set$vy($.add(p2.get$vy(), ay));
  }
};
...
$.main = function() {
  ...
  $.setRuntimeTypeInfo(t1, ({E: 'Particle'}));
  $.particles = t1;
  for (var i = 0; $.ltB(i, $.particleCount); ++i) {
    $.add$1($.particles, $.Particle$($.canvas, $.context));
  }
  $.document().get$window().setInterval$2($.draw, 4);
};
